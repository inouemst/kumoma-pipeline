...
使い方(第1段階遡及修正専用。stage2_fulltext_full.jsonを参照する):
...
def check_one(wid, guide_text):
    fulltext_full = json.load(open("stage2_fulltext_full.json", encoding="utf-8"))
    if wid not in fulltext_full:
        print(f"NG: 作品ID {wid} が stage2_fulltext_full.json に存在しません")
        return
