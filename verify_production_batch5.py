# -*- coding: utf-8 -*-
"""production_batch5.json の独立検証。batch1/2/3/4 と同じ手法。gates.py を直接実行する。"""
import json
import gates as g
import label_vocab as lv

fulltext_full = json.load(open("batch5_fulltext_full.json", encoding="utf-8"))
data = json.load(open("production_batch5.json", encoding="utf-8"))
...
for wid, r in fails:
    print(" NG:", wid, works.get(wid, {}).get("作品名"), r["第三層_固有名詞照合"]["本文に不在"],
          r["第四層_現代語ゲート"]["混入語"], r["第五層_文字数"], r["第五層_評価語"]["評価語"], r["第五層_結末示唆"]["結末示唆語"])
...
print("次バッチIDのうちproduction_batch5.jsonに存在しないもの:", missing)
