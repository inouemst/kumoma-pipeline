# -*- coding: utf-8 -*-
"""next_batch_ids.json(97件、第3バッチ)について、本文冒頭4,000字版と全文版を抽出する。
batch1/batch2と同じ手法(body_parser.pyの下請け関数使用)。body_parser.py自体は変更しない。"""
import json
import os
import re
...
for wid in ids:
    ...
    # 著者IDフィールドは共著の場合「276、1529」のように複数値を持つが、
    # 実際のディレクトリは本文URL自体に含まれる6桁IDを使う
    # （例：.../cards/001529/files/44909_29558.html → 001529）。
...
json.dump(fulltext_4000, open("batch3_fulltext_4000.json", "w", encoding="utf-8"), ensure_ascii=False)
json.dump(fulltext_full, open("batch3_fulltext_full.json", "w", encoding="utf-8"), ensure_ascii=False)
json.dump(meta, open("batch3_meta.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
json.dump(errors, open("batch3_errors.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
print("保存完了")
