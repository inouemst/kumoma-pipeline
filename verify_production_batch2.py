...
fulltext_full = json.load(open("batch2_fulltext_full.json", encoding="utf-8"))
data = json.load(open("production_batch2.json", encoding="utf-8"))
...
for wid, v in written.items():
    ...
    if title and len(title) >= 2 and title in v["案内文"]:
        print(" 作品名漏出の疑い:", wid, title, v["案内文"])
        title_leak += 1
print(f"作品名漏出の疑いのある件数: {title_leak}")
