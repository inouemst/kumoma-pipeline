...
使い方(第1段階遡及修正専用。stage3_fulltext_full.jsonを参照する):
  python gate_check_stage1.py <作品ID> "<案内文>"
      → その1件だけを全文照合して結果を表示

  python gate_check_stage1.py --all stage1_corrections.json
      → まとめて検証し、不合格の作品IDと理由だけを一覧表示
"""
import json
import sys
import gates as g
...
def check_one(wid, guide_text):
    fulltext_full = json.load(open("stage3_fulltext_full.json", encoding="utf-8"))
    if wid not in fulltext_full:
        print(f"NG: 作品ID {wid} が stage3_fulltext_full.json に存在しません")
        return
...
if __name__ == "__main__":
    if sys.argv[1] == "--all":
        ...
        wid = sys.argv[1]
        guide_text = sys.argv[2]
        check_one(wid, guide_text)
