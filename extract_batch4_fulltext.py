# -*- coding: utf-8 -*-
"""next_batch_ids.json(97件、第4バッチ)について、本文冒頭4,000字版と全文版を抽出する。
batch1/2/3と同じ手法(body_parser.pyの下請け関数使用)。body_parser.py自体は変更しない。"""
import json
import os
import re
...
json.dump(fulltext_4000, open("batch4_fulltext_4000.json", "w", encoding="utf-8"), ensure_ascii=False)
json.dump(fulltext_full, open("batch4_fulltext_full.json", "w", encoding="utf-8"), ensure_ascii=False)
json.dump(meta, open("batch4_meta.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
json.dump(errors, open("batch4_errors.json", "w", encoding="utf-8"), ensure_ascii=False, indent=1)
print("保存完了")
